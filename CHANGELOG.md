...
- Добавлена ручная привязка настоящего ФИО к `vk_user_id`.
- ФИО можно редактировать в списке студентов; синхронизация с VK его не перезаписывает.

### Fixed

- Исправлена IPv4 health-проверка frontend-контейнера.

### Documentation

- Добавлено пошаговое развёртывание по IP без домена.
